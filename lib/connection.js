...
const connect = async () => {
 ...
 /* process.on("unhandledRejection", async (err) => {
    ...
    const text = `\`\`\`Media Get unhandledRejection: \n${error}\`\`\``;
   const ownumber = ("919539412641@s.whatsapp.net");
    await conn.sendMessage(ownumber, { text });
    console.log(err);
  });
  process.on("uncaughtException", async (err) => {
    const error = util.format(err);
    const text = `\`\`\`Media Get uncaughtException: \n${error}\`\`\``;
    await conn.sendMessage(ownumber, { text });
    console.log(err);
  });*/
  return conn;
...
const handleConnectionUpdate = (conn) => async (s) => {
  ...
  if (connection === "connecting")
    ...
    let ownumber = ("919539412641@s.whatsapp.net")
    const str = `\`\`\`Cyber-366 connected\n\n\nVersion: 1.0.0\nTotal Plugins: ${totalPlugins}\nWorktype: ${workType}\`\`\``;
    conn.sendMessage(ownumber, { text: str });
